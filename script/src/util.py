...
def get_cm_account():
    ...
    cmAccount =  Account(
        AccountAddress.from_str(cm_public), 
        ed25519.PrivateKey.from_str(cm_private))
    return cmAccount
...
def mint(num_mints, amount_per_mint):
    ...
    user = cmAccount
    accountBalance = int (rest_client.account_balance(user.address().from_str(cmAccount.address)))
